...
mod tests {
    ...
    fn test_hyperplonk_helper<E: Pairing>(
        ...
        // generate pk and vks
        let (pk, vk) =
            <PolyIOP<E::ScalarField> as HyperPlonkSNARK<E, MercuryPCS<E>>>::preprocess(
                &index, &pcs_srs,
            )?;
        ...
        let proof = <PolyIOP<E::ScalarField> as HyperPlonkSNARK<E, MercuryPCS<E>>>::prove(
            &pk,
            &pi.0,
            &[w1.clone(), w2.clone()],
        )?;
        ...
        let _verify =
            <PolyIOP<E::ScalarField> as HyperPlonkSNARK<E, MercuryPCS<E>>>::verify(
                &vk, &pi.0, &proof,
            )?;
